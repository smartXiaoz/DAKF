...
%     end
    ...
    r = randn(m,N) * 0.1;
    v1=randn(m,N)*0.1; v2=randn(m,N)*100;
    rp=rand(1,N);  
    r = (rp<=0.9).*v1 + (rp>0.9).*v2;
%     r = stblrnd(1.5,0,1.5,0,m,N);%alpha 分布噪声
    %% ***
    Q = q * q'/N;
    R  = r * r'/N;
    R  = diag(diag(r * r'/N));
...
%                 t1(i,ii) = Vik(i,1) / (Vik(i,1) + Vik(i,2));
%                 t2(i,ii) = Vik(i,2) / (Vik(i,1) + Vik(i,2));
%              end
%              RE = t1(i,ii) * RR1 + t2(i,ii) * RR2;
%          end
