...
%             eey = r(:,ii);
            ...
            if ii > 1
                for  i = 1 : m 
                    ...
                    sigma = PP3(i,i);
                    Vik(i,3) = R_pai(3) * exp(-1 * (eey(i) - 0)^2/(2*sigma)) / sqrt(2*pi*sigma);
                    sumaa = (Vik(i,1) + Vik(i,2) + Vik(i,3));
                    if sumaa == 0
                        t1(i,ii) = 1;
                        t2(i,ii) = 0;
                    else
                        ...
                        t3(i,ii) = Vik(i,3) / sumaa;
                    end
%                     t2(i,ii) = 1 - t1(i,ii);
%                     tmp1 = t1(i,ii) * (RR1(i,i));
%                     tmp2 = t2(i,ii) * (RR2(i,i));
%                     Cy(i,i) = (tmp1 + tmp2);
                    Cy(i,i) = ((t1(i,ii) * (RR2(i,i)) + t2(i,ii) * (RR1(i,i)))); 
                    Cy(i,i) = ((t1(i,ii) * sqrt(RR2(i,i)) * sqrt(RR3(i,i)) + t2(i,ii) * sqrt(RR1(i,i)) * sqrt(RR3(i,i))  + t3(i,ii) * sqrt(RR1(i,i)) * sqrt(RR2(i,i)))/(R(i,i))); 
...
%                 vik3 = R_pai(3) * 1 / (2 * pi)^(m/2) / (abs(det(PP3)))^0.5 * exp(-0.5 * (eey-mu2)'*inv(PP3)*(eey-mu2));
%                 sumaa = (vik1 + vik2 + vik3);
...
% 
%                 end
%                 Cy = ((t1(ii) * sqrt(RR2)* sqrt(RR3) + t2(ii) * sqrt(RR1)* sqrt(RR3) + t3(ii) * sqrt(RR1)* sqrt(RR2)) ); 
% %                 PA1 = eye(m) * 5;
% %                 PA2 = eye(m) * 0.01;
