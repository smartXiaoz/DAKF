...
%             v(ii,:) = (numrand>=0.8&numrand<=0.9).*v2(ii,:) + (numrand<0.8).*v1(ii,:) + (numrand > 0.9).*v3(ii,:);
            v(ii,:) = v1(ii,:); % 1
            v(ii,:) = (numrand<=0.999).*v1(ii,:) + (numrand>0.999).*v2(ii,:);% 2
%             v(ii,:) = (numrand<=0.8).*v1(ii,:) + (numrand>0.8).*v2(ii,:);% 2
...
%     v = v1;
    % case 2
%     v = (rp<=0.999).*v1 + (rp>0.999).*v4;
    % case 3
%     v = (rp<=0.8).*v1 + (rp>0.8).*v2;
%     % case 4
%     v = (rp<=0.8).*v1 + (rp>0.8).*v3;
    % case 5
 
%     v1 = randn(m,NN)*0.1+ 0.5;
%     v2 = randn(m,NN)*5 -0.5;
%     v4 = gamrnd(4,5,m, NN);
%     v = (rp<=0.8).*v1 + (rp>0.8 & rp<0.95).*v2 + (rp>0.95).*v4;
    
    %% ***
    gmm=fitgmdist(v', 2,'RegularizationValue', 1e-8);
    ...
    P0_mckf_cv = p0;
    for i=2:Len
        [Pkk_MCKF3_CV,X_MCKF3_CV]=MCKF3(A1,G1,H,Q1,R,zA(:,i),x0_mckf_cv,P0_mckf_cv,p,2);
    ...
    P0_mckf_cv = p0;
    for i=2:Len
        [Pkk_MCKF3_CV,X_MCKF3_CV]=MCKF3(A1,G1,H,Q1,R,zA(:,i),x0_mckf_cv,P0_mckf_cv,p,10);
        X_MCKF_CV1(:,i)=X_MCKF3_CV;
        P0_mckf_cv=Pkk_MCKF3_CV;
        x0_mckf_cv=X_MCKF3_CV;
        Err_mckf1(i)=norm(X_MCKF_CV(:,i)-xA(:,i));
    end
    %% MEEKF CV单模型滤波
    x0_meekf_cv = x0_hj;
    P0_meekf_cv = p0;
    for i=2:Len
        [Pkk_MEEKF3_CV,X_MEEKF3_CV]=MEEKF(A1,G1,H,Q1,R,zA(:,i),x0_meekf_cv,P0_meekf_cv,p,1.5);
    ...
    P0_meekf_cv = p0;
    for i=2:Len
        [Pkk_MEEKF3_CV,X_MEEKF3_CV]=MEEKF(A1,G1,H,Q1,R,zA(:,i),x0_meekf_cv,P0_meekf_cv,p,6);
        X_MEEKF_CV1(:,i)=X_MEEKF3_CV;
        P0_meekf_cv=Pkk_MEEKF3_CV;
        x0_meekf_cv=X_MEEKF3_CV;
        Err_meekf1(i)=norm(X_MEEKF_CV(:,i)-xA(:,i));
    end
    %% EM_KF
    x0_emkf_cv = x0_hj;
    P0_emkf_cv = p0;
    for i=2:Len
        [Pkk_EMKF_CV,X_em_CV]=F_EMKF2(A1,G1,H,Q1,R,zA(:,i),x0_emkf_cv,P0_emkf_cv,p,RR1,RR2, R_pai,mu1,mu2);
        X_ikv(:,i)=X_em_CV;
    ...
    ex_mckf_cv2(mn,:)=X_MCKF_CV(1,:)-xA(1,:);
    ey_mckf_cv2(mn,:)=X_MCKF_CV(3,:)-xA(3,:);%MCKF_CV
    ex_meekf_cv15(mn,:)=X_MEEKF_CV(1,:)-xA(1,:);
    ey_meekf_cv15(mn,:)=X_MEEKF_CV(3,:)-xA(3,:);%MEEKF_CV
    ex_mckf_cv10(mn,:)=X_MCKF_CV1(1,:)-xA(1,:);
    ey_mckf_cv10(mn,:)=X_MCKF_CV1(3,:)-xA(3,:);%MCKF_CV
    ex_meekf_cv6(mn,:)=X_MEEKF_CV1(1,:)-xA(1,:);
    ey_meekf_cv6(mn,:)=X_MEEKF_CV1(3,:)-xA(3,:);%MEEKF_CV
...
%%  计算RMSE-均方根误差
EX_kf_cv=mean(sqrt(sum(ex_kf_cv.^2)/MC))
EY_kf_cv=mean(sqrt(sum(ey_kf_cv.^2)/MC))
EX_ikf_cv=mean(sqrt(sum(ex_ikf_cv.^2)/MC))
EY_ikf_cv=mean(sqrt(sum(ey_ikf_cv.^2)/MC))
EX_mckf_cv2=mean(sqrt(sum(ex_mckf_cv2.^2)/MC))
EY_mckf_cv10=mean(sqrt(sum(ey_mckf_cv10.^2)/MC))
EX_mckf_cv2=mean(sqrt(sum(ex_mckf_cv2.^2)/MC))
EY_mckf_cv10=mean(sqrt(sum(ey_mckf_cv10.^2)/MC))
EX_meekf_cv15=mean(sqrt(sum(ex_meekf_cv15.^2)/MC))
EY_meekf_cv6=mean(sqrt(sum(ey_meekf_cv6.^2)/MC))
EX_meekf_cv15=mean(sqrt(sum(ex_meekf_cv15.^2)/MC))
EY_meekf_cv6=mean(sqrt(sum(ey_meekf_cv6.^2)/MC))
